...
public class Writerbefore 
{
    ...
    private void sin()
	{
		...
		for (int i =0; i <row; i++)
		{
			for (int j = 0; j <row; j++) 
			{
				...
				a=Math.cos((bb)*((2*(xx))/3));
				b=Math.sin(((2*(n*(yy)))/t));
				c=Math.cos(((dd*(2*(xx)))/3));
				d=Math.sin((aa*(yy)/t));
				e=Math.cos(((ee*(2*(xx)))/3));
 ...
 // This method writes the generated matrix to a text file.
	private void write() throws IOException 
	{
		...
		BufferedWriter writer = new BufferedWriter(new FileWriter("cos/"+m+","+n+"cosbefore.txt"));// file name for generated file
